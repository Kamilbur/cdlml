...
i32_cases = [
    ...
    pytest.param(
        "identity_i32", [ctypes.c_int32], ctypes.c_int32, (2**31 - 1,), 2**31 - 1, id="max_i32"
    ),
    pytest.param(
        "identity_i32", [ctypes.c_int32], ctypes.c_int32, (-(2**31),), -(2**31), id="min_i32"
    ),
...
i64_cases = [
    ...
    pytest.param(
        "add_i64", [ctypes.c_int64] * 2, ctypes.c_int64, (-(10**15), 10**15), 0, id="add_cancel"
    ),
    pytest.param(
        "identity_i64", [ctypes.c_int64], ctypes.c_int64, (2**63 - 1,), 2**63 - 1, id="max_i64"
    ),
    pytest.param(
        "identity_i64", [ctypes.c_int64], ctypes.c_int64, (-(2**63),), -(2**63), id="min_i64"
    ),
]

u32_cases = [
    pytest.param(
        "add_u32", [ctypes.c_uint32] * 2, ctypes.c_uint32, (2**32 - 1, 1), 0, id="overflow_wrap"
    ),
    pytest.param(
        "add_u32", [ctypes.c_uint32] * 2, ctypes.c_uint32, (100, 200), 300, id="add_basic"
    ),
]

u64_cases = [
    pytest.param(
        "add_u64", [ctypes.c_uint64] * 2, ctypes.c_uint64, (2**63, 2**63), 0, id="overflow_wrap"
    ),
    pytest.param(
        "add_u64", [ctypes.c_uint64] * 2, ctypes.c_uint64, (2**64 - 1, 0), 2**64 - 1, id="max_u64"
    ),
]
...
cstr_cases = [
    ...
    pytest.param(
        "str_len", [ctypes.c_char_p], ctypes.c_uint32, (b"ab cd ef",), 8, id="with_spaces"
    ),
]
